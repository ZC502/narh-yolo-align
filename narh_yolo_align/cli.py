...
def _TODO_shared_postprocess(raw_preds, batch, *, conf: float, iou: float) -> List[List[Detection]]:
    """Placeholder for shared postprocess.

    Critical v0.1 rule:
    Both PyTorch raw outputs and ONNX raw outputs must pass through the same
    project-level postprocess function.

    Implement this with Ultralytics-compatible decoding + NMS for your target
    YOLO family before treating residual metrics as meaningful.
    """
    raise NotImplementedError(
        "Shared postprocess is intentionally left as a TODO. Do not compare residuals until PyTorch and ONNX outputs use identical postprocess."
    )


def build(args: argparse.Namespace) -> int:
    paths = list_images(args.source, max_images=args.max_images, seed=args.seed, shuffle=not args.no_shuffle)
    ...
    with (out_dir / "preprocess_hashes.jsonl").open("w", encoding="utf-8") as debug_f:
        for batch_idx, records in enumerate(iter_image_batches(paths, batch_size=args.batch_size), start=1):
            ...
            ref_dets = _TODO_shared_postprocess(ref_raw, pp, conf=args.conf, iou=args.iou)
            target_dets = _TODO_shared_postprocess(target_raw, pp, conf=args.conf, iou=args.iou)

            for rec, rd, td in zip(records, ref_dets, target_dets):
                match = greedy_match(rd, td, iou_match_threshold=args.match_iou, class_agnostic=True)
                residuals.append(classify_residual(
                    str(rec.path),
                    match,
    ...
    selected = build_calibration_set(grouped, out_dir=out_dir, max_images=args.max_calibration_images, representative_ratio=args.representative_ratio)
    write_report(residuals, selected, out_dir=out_dir, reference_backend="PyTorch FP32", target_backend="ONNX target")
...
def make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="narh-yolo-align", description="Build a label-free residual-guided calibration dataset for YOLO edge export.")
    ...
    p.add_argument("--conf", type=float, default=0.25, help="Common postprocess confidence threshold.")
    ...
    p.add_argument("--deploy-conf", type=float, default=0.25, help="Deployment confidence threshold.")
